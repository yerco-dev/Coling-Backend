...
public class LoginFunction
{
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "auth/login")] HttpRequest req)
    {
        try
        {
            _logger.LogInformation("Procesando solicitud de inicio de sesión.");
            ...
            var dto = JsonSerializer.Deserialize<LoginRequestDto>(requestBody, new JsonSerializerOptions
            {
        ...
        {
            _logger.LogError(ex, "Error inesperado al procesar el inicio de sesión.");
            return new ObjectResult(new
            {
                wasSuccessful = false,

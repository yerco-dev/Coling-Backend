...
public class GetWorkExperienceByIdFunction
{
    private readonly ILogger<GetWorkExperienceByIdFunction> _logger;
    private readonly GetWorkExperienceByIdUseCase _useCase;

    public GetWorkExperienceByIdFunction(
        ILogger<GetWorkExperienceByIdFunction> logger,
        GetWorkExperienceByIdUseCase useCase)
    {
        _logger = logger;
        _useCase = useCase;
    }
    ...
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "work-experience/{id}")] HttpRequest req,
        string id)
    {
        try
        {
            _logger.LogInformation($"Obteniendo experiencia laboral con ID: {id}");
        ...
        {
            _logger.LogError(ex, "Error inesperado al obtener experiencia laboral.");
            return new ObjectResult(new
            {
                wasSuccessful = false,

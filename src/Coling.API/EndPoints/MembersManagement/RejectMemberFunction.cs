...
public class RejectMemberFunction
{
    private readonly ILogger<RejectMemberFunction> _logger;
    private readonly RejectMemberUseCase _rejectMemberUseCase;
    private readonly IAuthorizationService _authorizationService;

    public RejectMemberFunction(
        ILogger<RejectMemberFunction> logger,
        RejectMemberUseCase rejectMemberUseCase,
        IAuthorizationService authorizationService)
    {
        _logger = logger;
        _rejectMemberUseCase = rejectMemberUseCase;
        _authorizationService = authorizationService;
    }

    [Function("RejectMember")]
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", "options", Route = "members/reject")] HttpRequest req,
        FunctionContext context)
    {
        try
        {
            ...
            var dto = JsonSerializer.Deserialize<RejectMemberDto>(requestBody, new JsonSerializerOptions
            {
            ...
            var result = await _rejectMemberUseCase.ExecuteAsync(dto);
        ...
        {
            _logger.LogError(ex, "Error inesperado al rechazar miembro.");
            return new ObjectResult(new
            {
                wasSuccessful = false,
